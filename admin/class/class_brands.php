...
class Marca
{
    ...
    public function modify($data)
    {
        ...
        $query = "SELECT activo FROM marca WHERE id_marca = ".$id_marca;
        $query = $this->con->query($query)->fetch(PDO::FETCH_ASSOC);

        if ($query['activo']==1) {
            $sql = "UPDATE marca SET activo = 0 WHERE id_marca = ".$id_marca;
        } else {
            $sql = "UPDATE marca SET activo = 1 WHERE id_marca = ".$id_marca;
        }
        $this->con->exec($sql);
