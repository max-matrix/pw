...
class Categoria
{
    ...
    {
        $query =  "SELECT * FROM categoria WHERE id_padre != 1";
        return $this->con->query($query);
    }

    public function getListSelected()
    {
        $query =  "SELECT id_categoria FROM categoria WHERE id_padre != 1";
        foreach ($this->con->query($query) as $key) {
            $max[] = $key['id_categoria'];
        }
        return $max;
    }
